...
class Cl_User
{
	...
	public function registration( array $data )
	{
		if( !empty( $data ) ){
			...
			$query = "INSERT INTO users (id, name, email, password, created,categories_id) VALUES (NULL, '$name', '$email', '$password', CURRENT_TIMESTAMP,'".$_POST['technology']."')";
	...
	public function forgetPassword( array $data )
	{
		if( !empty( $data ) ){
			...
			if(mysqli_query($this->_con, $query)){
				...
				$headers = "From: info@vchipdesign.com" . "\r\n" .
						"CC: vchipdesign@gmail.com";
	...
	public function getQuestions(array $data)
	{
		if( !empty( $data ) ){
			...
			$query = "INSERT INTO scores ( user_id,right_answer,category_id,subcat_id) VALUES ( '$user_id',0,'$category_id','$subcategory')";
			...
			$row = mysqli_query( $this->_con, "select * from questions where category_id=$category_id and subcat_id='".$subcategory."' ");
		...
		if( !empty( $data ) ){
			...
			while($result=mysqli_fetch_array($response)){
				if($result['answer']==$_POST[$result['id']]){
					$right_answer++;
				}else if($data[$result['id']]=='unsolved'){
					$unanswered++;
				}
				else{
					$wrong_answer++;
			...
			$marks= round(2*$right_answer - $wrong_answer*2/3, 2);
			$results = array();
